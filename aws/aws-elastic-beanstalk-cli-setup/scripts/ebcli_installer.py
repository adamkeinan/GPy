...
EXECUTABLE_WRAPPERS = {
    "bat": "\n".join(
        [
            ...
            ":done",
            "",
            "REM activate venv, call eb and deactivate venv",
...
def _exec_cmd(args):
    ...
    venv. `KeyboardInterrupt`s are in this parent process as showing
...
PIP_AND_VIRTUALENV_NOT_FOUND = " ".join(
    [
        'ERROR: Could not find "pip" and "venv" installed.'
        'Ensure "pip" and "venv" are installed and that they'
        "are in PATH before executing this script."
    ]
)

VIRTUALENV_DIR_NAME = ".ebcli-venv-env"

VIRTUALENV_NOT_FOUND = " ".join(
    [
        'ERROR: Could not find and "venv" installed. Ensure'
        "venv is installed and that it is in PATH before executing"
        "this script."
    ]
)
...
def _activate_venv(venv_location):
    """
	Function activates venv, ".ebcli-virtual-env", created apriori for the
	rest of the lifetime of this script.
	:param venv_location: the relative or absolute path to the location
						  where the venv, ".ebcli-virtual-env", was
...
@Step("Finishing up")
def _announce_success(venv_location, hide_export_recommendation):
    """
	...
	scripts but only when `venv_location` is not already in PATH.
	...
	:return: None
	"""
    new_location = _eb_wrapper_location(venv_location)
    ...
    if sys.platform.startswith("win32"):
        ...
        print(
            "\033[38;5;{color_number}m{message}\033[0m".format(
                color_number=color_number, message=message,
            )
        )
...
@Step("Creating exclusive venv for EBCLI")
def _create_venv(venv_executable, venv_location, python_installation, quiet):
    """
	Function creates a new venv at path `venv_location`
	using the Python at path `python_installation`, if one is provided.
	If `venv_location` is not provided, the user's HOME directory
	is assumed as the location to create the venv in. If
	`python_installation` is not provided, venv attempts to
	...
	Prior to creation of the venv, this function checks whether
	...
	:param venv_executable: the name of the venv executable
	:param venv_location: the relative or absolute path to the location
								where the venv, ".ebcli-virtual-env", must
								be created.
	:param python_installation: the relative or absolute path to the location
								of a Python executable to use to create the
								venv with
	:param quiet: whether to display the output of venv creation in
				  STDOUT or not

	:return the relative or absolute path to the location where the
			venv, ".ebcli-virtual-env", was created.
	"""
    venv_location = venv_location or _user_local_directory()
    venv_directory = os.path.join(venv_location, VIRTUALENV_DIR_NAME)
    python_installation = python_installation or sys.executable

    if os.path.exists(venv_directory) and not _directory_was_created_by_installer(
        venv_directory
    ):
        _error(
            'Installation cannot proceed because "{venv_location}" already exists '
            "but was not created by this EBCLI installer."
            "\n"
            "\n"
            "You can either:\n"
            "\n"
            '1. Delete "{venv_location}" after verifying you don\'t need it; OR\n'
            "2. Specify an alternate location to install the EBCLI and its artifacts in "
            "using the `--location` argument of this script .\n".format(
                venv_location=venv_directory
            )
        )

    venv_args = [venv_executable or "venv", '"{}"'.format(venv_directory)]

    python_installation and venv_args.extend(["-p", '"{}"'.format(python_installation)])

    if _exec_cmd(venv_args, quiet) != 0:
        exit(1)

    _add_ebcli_stamp(venv_directory)

    return venv_location


@Step("Locating venv installation")
def _locate_venv_executable():
    """
	Function attempts to find the location at which `venv` is installed.
	...
	or both of `pip` and `venv` are missing.
	...
	:side-effect: script will exit with a non-0 return code if a
				  venv and/or pip executables haven't been found.
	"""
    venv_executables = ["venv"]

    if sys.platform.startswith("win32"):
        venv_executables += ["venv.cmd", "venv.exe"]
    venv_executable = None
    for _venv_executable in venv_executables:
        if _executable_found(_venv_executable, True):
...
def _generate_ebcli_wrappers(venv_location):
    """
	Function generates:
	...
	:param venv_location: the relative or absolute path to the location
						  where the venv, ".ebcli-virtual-env",
						  exists.
	:return None
	"""
    executables_dir = _eb_wrapper_location(venv_location)
    ...
    if sys.platform.startswith("win32"):
        with open(ebcli_ps1_script_path, "w") as script:
            script.write(_powershell_script_body(venv_location))

        with open(ebcli_bat_script_path, "w") as script:
            script.write(_bat_script_body(venv_location))
    else:
        with open(ebcli_script_path, "w") as script:
            script.write(_python_script_body(venv_location))
...
def _install_ebcli(quiet, version, ebcli_source):
    """
	Function installs the awsebcli presumably within the venv,
...
def _add_ebcli_stamp(venv_directory):
    """
	...
	:param venv_directory: The directory where the EBCLI and its artifacts
	will be installed
	:return: None
	"""
    with open(os.path.join(venv_directory, EBCLI_INSTALLER_STAMP), "w") as file:
        file.write("\n")


def _bat_script_body(venv_location):
    """
	...
	the venv, ".ebcli-virtual-env", created apriori.
	:param venv_location: the relative or absolute path to the location
						  where the venv, ".ebcli-virtual-env", was
						  created.
	:return: None
    ...
    return EXECUTABLE_WRAPPERS["bat"].format(
        bin_location=_original_eb_location(venv_location)
    )


def _directory_was_created_by_installer(venv_directory):
    """
	Function checks whether `venv_directory` was previously created
	by this script.

	:param venv_directory: The directory where the EBCLI and its
								 artifacts will be installed.
	:return: Boolean indicating whether `venv_directory` was created
			 by this script or not.
	"""
    return os.path.exists(os.path.join(venv_directory, EBCLI_INSTALLER_STAMP))


def _eb_wrapper_location(venv_location):
    """
	Function returns the location of the directory within the venv,
	".ebcli-virtual-env", where the wrapper of the `eb` executable should
	be installed. This is `executables` on all OSes.
	:param venv_location: the relative or absolute path to the location
						  where the venv, ".ebcli-virtual-env", was
						  created.
	:return: the location of the directory within the venv where
			 the wrapper of the `eb` executable should be installed.
	"""
    return os.path.join(
        os.path.abspath(venv_location), VIRTUALENV_DIR_NAME, "executables"
    )


def _ensure_not_inside_venv_to_begin_with():
    """
	Function checks whether the `VIRTUAL_ENV` environment variable has
	already been set in effect checking whether a venv is presently
	active in the context of this shell. If so, this script is exited
	immediately with a non-0 exit code.

	Rationale:
		1. when a venv is entered current `PATH` is saved
		2. real `PATH` is amended so that the `bin`/`Scripts` directory inside
		   the venv is at the head of PATH
		...
		4. all of the changes made to PATH will be lost upon deactivation
		   of the venv thereby leading to unpredictable behaviour.

	:return: None
	:side-effect: script will exit with a non-0 return code if a
				  venv has already been activated within the shell.
	"""
...
def _original_eb_location(venv_location):
    """
	Function returns the location of the directory within the venv,
    ...
    return os.path.join(
        os.path.abspath(venv_location), VIRTUALENV_DIR_NAME, scripts_directory
    )
...
def _parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        "This program creates an isolated venv solely meant for invoking "
    ...
    parser.add_argument(
        "-e",
        "--venv-executable",
        help="path to the venv installation to use to create the EBCLI's venv",
    )
    parser.add_argument(
        "-i",
...
def _powershell_script_body(venv_location):
    """
...
def _python_script_body(venv_location):
    """
...
if __name__ == "__main__":
    _ensure_not_inside_venv_to_begin_with()
    arguments_context = _parse_arguments()
    venv = arguments_context.venv_executable or _locate_venv_executable()
    venv_location = _create_venv(
        venv,
        arguments_context.location,
        arguments_context.python_installation,
        arguments_context.quiet,
    )
    _activate_venv(venv_location)
    _install_ebcli(
        arguments_context.quiet,
        arguments_context.version,
        arguments_context.ebcli_source,
    )
    _generate_ebcli_wrappers(venv_location)
    _announce_success(venv_location, arguments_context.hide_export_recommendation)
