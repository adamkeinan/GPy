...
CHROMEDRIVER_PATH = "/locale/chromedriver/chromedriver"
X_SERVER_DISPLAY = ":0"
...
"""
        ...
        self._server = chromedriver_server(
        ...
        urllib2.urlopen(
            "http://localhost:%i/json/new" % utils.get_chrome_remote_debugging_port()
        )
        chromeOptions = {
            "debuggerAddress": (
        ...
        try:
            self.driver = webdriver.Remote(
                command_executor=self._server.url, desired_capabilities=capabilities
            )
        except NameError:
            logging.error("selenium module failed to be imported.")
            raise
...
"""
    ...
    def __init__(
        self, exe_path, port=None, skip_cleanup=False, url_base=None, extra_args=None
    ):
...
"""
        ...
        logging.debug("Chrome Driver server is up and listening at port %d.", port)
        if not skip_cleanup:
